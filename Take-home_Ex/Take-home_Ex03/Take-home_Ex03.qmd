...
## edges
```{r}
mc3_edges_filt <- mc3_edges %>% select(type, source, target, start_date, end_date)
glimpse(mc3_edges_filt)
```

```{r}
# Check all end data is N/A or in 2035 YEAR
all_end_dates_valid <- mc3_edges_filt %>%
...
** We can ignore the end_data, since all of them all over 2024 or N/A **
...
(1) before 2033-10-29
(2) between 2033-10-29 and 203d'ce5-05-25
(3) after 2035-05-25
...
```
Make more bins, e.g.,using month as boundry

## 4.1 explore

```{r}
# Add a year-month column for binning
mc3_edges_select <- mc3_edges_select %>%
  mutate(year_month = floor_date(as.Date(start_date), "month"))

# Define the range of months to create bins
start_date <- as.Date("2033-10-01")
end_date <- as.Date("2035-06-01")
...
# Loop through each month and filter edges
for (month in all_months) {
  edges_month <- mc3_edges_select %>%
    filter(year_month == month) %>%
    filter(str_detect(from, "SouthSeafood") | str_detect(to, "SouthSeafood"))
  
  # If there are edges, store them
  if (nrow(edges_month) > 0) {
    edges_list[[as.character(month)]] <- edges_month
    nodes_month <- unique(c(edges_month$from, edges_month$to))
    nodes_list[[as.character(month)]] <- data.frame(id = nodes_month)
  }
}

# Print the edges for each month (to see the results)
for (month in names(edges_list)) {
  print(paste("Month:", month))
  print(edges_list[[month]])
}

# Plotting the networks for each month
for (month in names(edges_list)) {
  nodes <- nodes_list[[month]]
  edges <- data.frame(from = edges_list[[month]]$from, 
                      to = edges_list[[month]]$to, 
                      label = edges_list[[month]]$type, 
                      title = paste("Start Date:", edges_list[[month]]$start_date, 
                                    "<br>End Date:", edges_list[[month]]$end_date))
  
  visNetwork(nodes, edges, main = paste("Network for month:", month)) %>%
    visEdges(arrows = "to") %>%
