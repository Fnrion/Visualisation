...
```{r}
 mc3_edges_select <- mc3_edges_clean %>%
  filter(source %in% mc3_nodes_clean$id | target %in% mc3_nodes_select$id) %>%
  distinct() %>%
  rename(from = source,
         to = target)
```
...
mc3_nodes_combined$group <- ifelse(mc3_nodes_combined$id %in% mc3_nodes_select$id, "Organization", "Person")
...
visNetwork(mc3_nodes_combined, mc3_edges_select_high) %>%
  ...
  visLegend() %>%
  visEdges(arrows = "to", label = "start_date") %>% # Include start_date as label on edges
  visOptions(
...
```{r}
# Filter edges based on start_date before 2023
filtered_edges <- mc3_edges_select %>%
  filter(as.Date(start_date) < as.Date("2023-01-01"))

# Filter edges to include any that contain "SouthSeafood"
southseafood_edges <- filtered_edges %>%
  filter(str_detect(from, "SouthSeafood") | str_detect(to, "SouthSeafood"))
glimpse(southseafood_edges)
```

::: callout-idea
If the dataset is correct, check the timeline first
:::
...
\*\* It's interesting to find that the event seems happen in future? \*\*
...
# Filter nodes for organizations
mc3_nodes_select <- mc3_nodes_clean %>%
  filter(grepl("Entity\\.Organization", type, ignore.case = TRUE))

# Define date range for filtering edges
start_period <- as.Date("2000-01-01")
end_period <- as.Date("2025-12-31")

# Extract edges related to organization nodes
mc3_edges_select <- mc3_edges_clean %>%
...
# Extract edges with high connection
mc3_edges_select_high <- mc3_edges_select %>%
  group_by(from) %>%
  mutate(count = n()) %>%
  filter(count >= 8) %>%
...
# Filter edges for dates before 2033-10-29
edges_before_2033 <- mc3_edges_select_high %>%
  filter(start_date < as.Date("2033-10-29"))

# Filter edges for dates after 2035-05-25
edges_after_2035 <- mc3_edges_select_high %>%
  filter(start_date > as.Date("2035-05-25"))

# Prepare nodes for each period
nodes_before_2033 <- bind_rows(
  edges_before_2033 %>%
    select(from) %>%
    distinct(from) %>%
    rename(id = from),
...
nodes_before_2033$group <- ifelse(nodes_before_2033$id %in% mc3_nodes_select$id, "Entity Organization", "Entity.Person")
nodes_after_2035$group <- ifelse(nodes_after_2035$id %in% mc3_nodes_select$id, "Entity Organization", "Entity.Person")

# Visualize network for edges before 2033-10-29
visNetwork(nodes_before_2033, edges_before_2033) %>%
  visPhysics(solver = "forceAtlas2Based",
             forceAtlas2Based = list(gravitationalConstant = -100)) %>%
  visIgraphLayout(layout = "layout_with_fr") %>%
...
# Visualize network for edges after 2035-05-25
visNetwork(nodes_after_2035, edges_after_2035) %>%
  visPhysics(solver = "forceAtlas2Based",
             forceAtlas2Based = list(gravitationalConstant = -100)) %>%
  visIgraphLayout(layout = "layout_with_fr") %>%
  visEdges(arrows = "to", label = "start_date") %>%
