...
```{r}
# Clean and select columns from mc3_data$edges
mc3_edges_clean <- as_tibble(mc3_edges_filt) %>% 
  mutate(
    start_date = as_datetime(start_date),
    end_date = as_datetime(end_date)
...
# Extract edges related to selected nodes
mc3_edges_select <- mc3_edges_filtered_dates %>%
  filter(source %in% mc3_nodes_select$id | target %in% mc3_nodes_select$id) %>%
  distinct() %>%
  rename(from = source,
         to = target)

# Extract edges with high connection (count >= 20)
mc3_edges_select_high <- mc3_edges_select %>%
  group_by(from) %>%
  mutate(count = n()) %>%
  filter(count >= 20) %>%
...
# Get distinct source and target nodes
mc3_nodes_source <- mc3_edges_select_high %>%
  distinct(from) %>%
  rename(id = from)
...
# Assign group based on subtype
mc3_nodes_combined$group <- mc3_nodes_combined$subtype
...
visNetwork(mc3_nodes_combined, mc3_edges_select_high) %>%
  visPhysics(solver = "forceAtlas2Based",
             forceAtlas2Based = list(gravitationalConstant = -100)) %>%
  visIgraphLayout(layout = "layout_with_fr") %>%
  visGroups(groupname = "FishingCompany", color = "yellow") %>%
  visGroups(groupname = "SomeOtherSubtype", color = "grey") %>%
  visLegend() %>%
  visEdges(arrows = "to", label = "start_date") %>%
  visOptions(
    highlightNearest = list(enabled = TRUE, degree = 2, hover = TRUE),
